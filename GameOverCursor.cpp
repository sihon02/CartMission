...
#define TEXTURE_WIDTH		(350)	// テクスチャサイズ
#define TEXTURE_HEIGHT		(170)	 
#define TEXTURE_DRAW		(1)		// 桁数

void GameOverCursor::Init()
{
	m_W = TEXTURE_WIDTH;
	m_H = TEXTURE_HEIGHT;
	...
	m_Position = D3DXVECTOR3(265.0f, 440.0f, 0.0f);
...
void GameOverCursor::Update()
{

	//*********カーソル移動***********//

	if (Input::GetKeyTrigger('D') && m_Cursor == 0)
	{
		m_Position.x += 450.0f;
		m_Cursor = 1;
	}

	if (Input::GetKeyTrigger('A') && m_Cursor == 1)
	{
		m_Position.x -= 450.0f;
		m_Cursor = 0;
	}
